   ...
   public static void enterScores(Connection conn)
   {
      ...
      try
      {
         ...
         do
         {
            ...
            result = stmt.executeQuery("SELECT * FROM Games " +
                                       "WHERE GameNumber = " +
                                       gameNumber);
            ...
            if(result.next())
            {
               ...
               // game number
               stmt.executeUpdate("UPDATE Games SET " +
                                  "HomeTeamScore = " +
                                  score1 +
                                  " WHERE GameNumber = " +
                                  gameNumber);
               ...
               if(score1 < score2)
               {
                  ...
                  result = stmt.executeQuery("SELECT Losses " +
                                             "FROM Teams " +
                                             " WHERE TeamName = " +
                                             "'" + homeTeam + "'");
                  if(result.next())
                  {
                     ...
                     stmt.executeUpdate("UPDATE Teams " +
                                        "SET Losses = " +
                                        (losses + 1) +
                                        " WHERE TeamName = " +
                                        "'" + homeTeam + "'");
                  ...
                  result = stmt.executeQuery("SELECT Wins " +
                                             "FROM Teams " +
                                             " WHERE TeamName = " +
                                             "'" + visitingTeam + "'");
               ...
               else
               {
                  ...
                  // Do the same for the visiting team
                  int ties;
                  result = stmt.executeQuery("SELECT Ties " +
                                             "FROM Teams " +
                                             " WHERE TeamName = " +
                                             "'" + homeTeam + "'");
